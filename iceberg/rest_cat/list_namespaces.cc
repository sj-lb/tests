...
int main(int argc, char** argv) {
	...
	// Prompt user for namespace to list tables
	std::cout << "\nEnter namespace to list tables (press Enter to skip, use dot notation for nested namespaces): ";
	std::string namespace_str;
	std::getline(std::cin, namespace_str);

	if (!namespace_str.empty()) {
		// Parse namespace from dot-separated string
		iceberg::Namespace target_ns;
		size_t start = 0;
		size_t dot_pos;
		while ((dot_pos = namespace_str.find('.', start)) != std::string::npos) {
			target_ns.levels.push_back(namespace_str.substr(start, dot_pos - start));
			start = dot_pos + 1;
		}
		target_ns.levels.push_back(namespace_str.substr(start));

		std::cout << "\nListing tables in namespace: ";
		for (size_t i = 0; i < target_ns.levels.size(); ++i) {
			if (i > 0) std::cout << ".";
			std::cout << target_ns.levels[i];
		}
		std::cout << std::endl;

		auto tables_result = catalog->ListTables(target_ns);
		if (!tables_result.has_value()) {
			std::cerr << "Failed to list tables: "
						<< tables_result.error().message << std::endl;
			return 1;
		}
		...
		if (tables.empty()) {
			std::cout << "No tables found in this namespace." << std::endl;
		} else {
			std::cout << "\nFound " << tables.size() << " table(s):" << std::endl;
			std::cout << "----------------------------------------" << std::endl;
			
			for (const auto& table : tables) {
				std::cout << "  ";
				// Print namespace
				for (size_t i = 0; i < table.ns.levels.size(); ++i) {
					if (i > 0) std::cout << ".";
					std::cout << table.ns.levels[i];
				}
				if (!table.ns.levels.empty()) std::cout << ".";
